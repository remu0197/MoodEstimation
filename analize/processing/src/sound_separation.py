...
class SoundSeparation :
  ...
  def separate(self,R_t,n_iter_t,R,n_iter,predict_pathes, train_path, r_pathes):
    ...
    for i, predict in enumerate(predict_pathes):
      ...
      # CNMF
      csnmf = self.__cnmf(np.abs(s_b), R=R, T=10, n_iter=n_iter)
      result = librosa.istft(csnmf[0] * np.cos(np.anble(s_b) + 1j * np.sin(np.angle(s_b))))
  ...
  def __cnmf(self, Y, R, T, n_iter):
    ...
    Ht = [np.random.rand(M, R) for _ in range(T)]
    U = np.random.rand(R, N)
    Ut = self.__get_t_array(array=U, T=T, is_right=True)
    Yt = self.__get_t_array(array=Y, T=T, is_right=True)

    Yh = np.dot(Ht[0],Ut[0])
    for t in range(1,T):
      Yh += np.dot(Ht[t],Ut[t])
    Yht = self.__get_t_array(array=Yh, T=T, is_right=True)
    ...
    for it in range(n_iter):
      ...
      for t in range(T):
        ...
        Ut[t] *= np.dot(Ht[t].T, Yt[t]) / np.dot(Ht[t].T, Yht[t])
